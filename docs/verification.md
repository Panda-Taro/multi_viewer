...
> **未実装/要確認**: `compositor/compose.py`はMTL公式FFmpegプラグイン
> (mtl_st20p)のデフォルトフォーマット(1920x1080/yuv422p10le/59.94fps、
> 本システムの既定フォーマットと一致)を前提としており、NMOS SDPで
> それ以外のフォーマットが指定された場合の`-video_size`/`-pix_fmt`/`-fps`
> オプションへの動的マッピングは未実装。実機検証時、非デフォルト
> フォーマットで受信する場合はこの点を確認・対応すること。
